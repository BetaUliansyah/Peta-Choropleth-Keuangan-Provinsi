...
def display_data_filter():
    return st.sidebar.radio('Data', data_list, index=0)
    
def display_axis_y_filter():
    return st.sidebar.radio('Data Axis Y', data_list, index=1)
    
def display_map(data, df, year, top_slider=5):
    st.header(f'Peta {data} tahun {year}')
    df = df[(df['tahun'] == year)]

    # merged geojson and df
    # Ensure both DataFrames have the same data type for the merge key
    map_gpd = gpd.read_file('map/indonesia_38_provinsi_bps.json')
    ...
    tooltip = GeoJsonTooltip(
        ...
        aliases=["Provinsi :", "Kode Provinsi:", f"{data}:"], # dari map geojson juga
        style=(
    ...
    for feature in choropleth.geojson.data['features']:
        ...
        state_name = feature['properties']['wilayah']
...
def display_ranking(data, df, year, state_name=''):
    st.header(f'Ranking {data} {state_name} tahun {year}')
...
def display_trend(data, df, state_name):
    if state_name == '':
        st.warning(f'Pilih daerah terlebih dahulu')    
    tahun_awal = df['tahun'].min()
    tahun_akhir = df['tahun'].max()
    st.header(f'Trend {data} {state_name} tahun {tahun_awal}-{tahun_akhir}')
...
def main():
    ...
    with tab1:
        display_ranking(data, df, year, state_name)
    with tab2:
        display_trend(data, df, state_name)
    # with tab3:
    ...
    #with col2:
        ...
        #    print("Test")
    #with col3:
    #    display_facts(df_ipm, year, state_name, 'nilai', f'Nilai', string_format='{:,}')
